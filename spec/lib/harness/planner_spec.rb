...
RSpec.describe Harness::Planner do
  ...
  def adapter(body, model: "fake-model")
    Class.new do
      define_method(:complete) { |system:, user:, schema: nil, max_tokens: nil, temperature: nil, thinking: nil| body.respond_to?(:call) ? body.call(user) : body }
  ...
  describe "failure isolation" do
    it "captures a raised adapter error instead of propagating" do
      boom = Class.new do
        def complete(system:, user:, schema: nil, max_tokens: nil, temperature: nil, thinking: nil) = raise "network down"
