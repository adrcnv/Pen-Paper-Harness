...
RSpec.describe "Harness::Tools propose family" do
  ...
  describe Harness::Tools::ProposeEvent do
    ...
    describe "creates_pending_appearance (bundled future-consequence row)" do
      ...
      it "creates a class-4 actor appearance from the same call" do
        expect {
          result = described_class.new.call(
            {
              "scope"   => "local",
              "trigger" => "stole his coat",
              "participants" => [
                { "character_id" => player.id, "role" => "actor" },
                { "character_id" => joey.id,   "role" => "victim" }
              ],
              "creates_pending_appearance" => {
                "target_character_id" => player.id,
                "actor_character_id"  => joey.id,
                "origin_character_id" => joey.id,
                "intent_text"         => "demands his coat back",
                "scope"               => "city",
                "earliest_at_offset_minutes" => 60
              }
            },
            context
          )
          expect(result["pending_appearance"]).to include("id", "earliest_at" => 161)
        }.to change(PendingAppearance, :count).by(1)

        pa = PendingAppearance.last
        expect(pa.target_character_id).to eq(player.id)
        expect(pa.actor_character_id).to eq(joey.id)
        expect(pa.origin_character_id).to eq(joey.id)
        expect(pa.intent_text).to eq("demands his coat back")
        expect(pa.scope).to eq("city")
        expect(pa.anchor_location_id).to eq(tavern.id)
        expect(pa.triggered_by_event_id).to eq(Event.last.id)
      end

      it "creates a faceless faction appearance (no actor specifier)" do
        result = described_class.new.call(
          {
            "scope"   => "local",
            "trigger" => "robbed the guild's strongbox",
            "participants" => [ { "character_id" => player.id, "role" => "actor" } ],
            "creates_pending_appearance" => {
              "target_character_id" => player.id,
              "origin_faction_id"   => guild.id,
              "intent_text"         => "demands return of stolen goods",
              "scope"               => "anywhere",
              "earliest_at_offset_minutes" => 1440
            }
          },
          context
        )
        expect(result["pending_appearance"]["id"]).to be_present

        pa = PendingAppearance.last
        expect(pa.origin_faction_id).to eq(guild.id)
        expect(pa.actor_character_id).to be_nil
        expect(pa.scope).to eq("anywhere")
        expect(pa.anchor_location_id).to be_nil  # anywhere doesn't need anchor
        expect(pa.earliest_at).to eq(101 + 1440)
      end

      it "defaults scope=city and offset=60 when not specified" do
        described_class.new.call(
          {
            "scope"   => "local",
            "trigger" => "x",
        ...
        pa = PendingAppearance.last
        expect(pa.scope).to eq("city")
        expect(pa.earliest_at).to eq(101 + 60)
      end

      it "uses anchor_location_id override when provided" do
        described_class.new.call(
          {
            "scope" => "local", "trigger" => "x",
            "creates_pending_appearance" => {
              "target_character_id" => player.id,
              "origin_faction_id"   => guild.id,
              "intent_text"         => "x",
              "anchor_location_id"  => warehouse.id
            }
          },
          context
        )
        expect(PendingAppearance.last.anchor_location_id).to eq(warehouse.id)
      end

      it "returns appearance error but keeps the event committed when validation fails" do
        result = nil
        expect {
          result = described_class.new.call(
            {
              ...
              "creates_pending_appearance" => {
                "target_character_id" => player.id,
                "intent_text"         => "x"
                # No actor and no origin_faction → faceless invalid
              }
            },
            context
          )
        }.to change(Event, :count).by(1).and(change(PendingAppearance, :count).by(0))

        expect(result["event_id"]).to be_present
        expect(result["pending_appearance"]["error"]).to match(/origin_faction_id/)
      end

      it "rejects unknown target_character_id" do
        result = described_class.new.call(
          {
            "scope"   => "local", "trigger" => "x",
            "creates_pending_appearance" => {
              "target_character_id" => 99_999,
              "origin_faction_id"   => guild.id,
              "intent_text"         => "x"
            }
          },
        ...
        expect(result["pending_appearance"]["error"]).to match(/no character with id=99999/)
      end
        ...
        result = described_class.new.call(
          {
            ...
            "creates_pending_appearance" => {
              ...
              "origin_faction_id"   => guild.id,
              "intent_text"         => "x",
              "scope"               => "kingdom"  # not in PA scope set
            }
          },
          context
        )
        expect(result["pending_appearance"]["error"]).to match(/scope must be one of/)
      end

      it "no-ops cleanly when creates_pending_appearance is omitted" do
        result = described_class.new.call(
          { "scope" => "local", "trigger" => "ordinary thing" },
          context
        )
        expect(result["pending_appearance"]).to be_nil
        expect(PendingAppearance.count).to eq(0)
      end
    end
  end
